...
from codex_rollout_capture import find_thread, text_from_content  # noqa: E402
...
def mirror_rollout_item(run_id: str, item: dict[str, Any]) -> int:
    ...
    if event_type == "session_meta":
        ...
        count = 0
        if base_text:
            record_prompt(run_id, "system", base_text, "base_instructions", "codex-live")
            count += 1
        append_event(run_id, "decision", "Codex session metadata imported", {"source": "codex-live", "timestamp": timestamp, "payload": session_payload})
        return count + 1
    ...
    if event_type == "turn_context" and isinstance(payload, dict):
        ...
        if isinstance(user_instructions, str) and user_instructions.strip():
            record_prompt(run_id, "developer", user_instructions, "project_instructions", "codex-live")
            count += 1
        append_event(run_id, "decision", "Codex turn context imported", {"source": "codex-live", "timestamp": timestamp, "payload": payload})
        return count + 1
    ...
    if event_type == "response_item" and isinstance(payload, dict):
        ...
        if payload_type == "message" and role in {"user", "developer", "system"}:
            record_prompt(run_id, str(role), content, "conversation", "codex-live")
            return 1
        if payload_type == "message" and role in {"assistant", "model"}:
            record_model_response(run_id, content, "codex-live")
            return 1
        if payload_type in {"function_call", "tool_call", "local_shell_call"}:
            ...
            append_event(run_id, "tool_call", f"{name} 호출", {"source": "codex-live", "timestamp": timestamp, "payload": payload})
            return 1
        if payload_type in {"function_call_output", "tool_result", "local_shell_call_output"}:
            append_event(run_id, "tool_result", "도구 실행 결과", {"source": "codex-live", "timestamp": timestamp, "payload": payload})
            return 1
        append_event(run_id, "decision", compact(content or str(payload_type)), {"source": "codex-live", "timestamp": timestamp, "payload": payload})
        return 1
    ...
    if event_type == "event_msg" and isinstance(payload, dict):
        ...
        if payload_type in {"task_completed", "turn_completed", "task_finished", "task_complete"}:
            append_event(run_id, "outcome", "Codex turn completed", {"status": "completed", "source": "codex-live", "timestamp": timestamp, "payload": payload})
        elif payload_type in {"task_failed", "turn_failed"}:
            append_event(run_id, "outcome", "Codex turn failed", {"status": "failed", "source": "codex-live", "timestamp": timestamp, "payload": payload})
        elif payload_type == "token_count":
            token_info = payload.get("info") or {}
            total = token_info.get("total_token_usage") or {}
            tokens = total.get("total_tokens")
            record_metric(run_id, token_count=tokens if isinstance(tokens, int) else None)
            append_event(run_id, "decision", "Codex token usage updated", {"source": "codex-live", "timestamp": timestamp, "payload": payload})
        else:
            append_event(run_id, "decision", payload_type or "Codex event", {"source": "codex-live", "timestamp": timestamp, "payload": payload})
        return 1

    append_event(run_id, "decision", compact(str(event_type)), {"source": "codex-live", "timestamp": timestamp, "payload": payload})
    return 1
...
def watch(args: argparse.Namespace) -> None:
    ...
    state.update({"status": "starting", "run_id": run_id, "mission": args.mission})
    write_json(watcher_state_path(run_id), state)
    print(json.dumps({"status": "started", "run_id": run_id}, ensure_ascii=False), flush=True)

    while True:
        try:
            result = run_once(args)
            print(json.dumps({"status": "tick", "run_id": run_id, "events_imported": result["events_imported"]}, ensure_ascii=False), flush=True)
